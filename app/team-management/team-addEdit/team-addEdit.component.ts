...
export class TeamAddEditComponent implements OnInit, OnDestroy {
  ...
  onSave() {
    ...
    if (this.teamForm.touched && this.teamForm.valid) {
      ...
      this.teamManagementService.saveTeam(team, this.inEdit()).subscribe(
        response => {
          this.teamManagementService.resetTable(true);
  ...
  onInactivate() {
    this.confirmationService.confirm({
      message:
        (this.teamForm.dirty ? 'This function will inactivate the team and any unsaved changes will be lost. ' :
          'This function will inactivate the team. ') + 'Do you wish to proceed?',
      accept: () => {
