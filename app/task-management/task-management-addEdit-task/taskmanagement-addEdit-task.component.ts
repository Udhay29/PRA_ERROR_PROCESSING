...
export class TaskmanagementAddEditTaskComponent implements OnInit {
  ...
  taskForm: FormGroup = this.fb.group(
    {
      assignmentTitle: ['', [Validators.required, Validators.maxLength(120)]],
      category: ['', Validators.required],
      responsibleTeams: ['', Validators.required],
      assignToList: this.fb.array(
        [],
        [
          Validators.required,
          TaskManagementValidators.noDuplicateValuePairsValidator
        ]
      ),
      workAssignmentList: this.fb.array(
        [],
        [
          Validators.required,
          TaskManagementValidators.noDuplicateValuePairsValidator
  ...
  constructor(
    ...
    this.teamSearchSubject
      .pipe(
        switchMap(input =>
          this.teamManagementService.findTeamWithNameContaining(input)
        )
      )
      .subscribe(teams => {
    ...
    if (!this.taskId) {
      ...
      this.task.assignedTo.forEach(team => {
        this.formAssignToArray.push(
          this.fb.group({
            ...
            teamID: team.teamID,
            teamMemberTaskAssignmentRoleAssociationID:
              team.teamMemberTaskAssignmentRoleAssociationID,
      ...
      this.task.workAssignments.forEach(workAssignment => {
        const group: any = this.fb.group({
          type: [
            workAssignment.taskResponsibilityTypeDescription,
            Validators.required
          ],
          value: [
            workAssignment.taskAssignmentResponsibilityDetail,
            Validators.required
          ]
        });
        this.taskResponsibilityTypeChange(
          workAssignment.taskResponsibilityTypeDescription,
          group
        );
  ...
  confirmActionIfTouched = (action: Function): void => {
    if (this.taskForm.touched) {
      this.confirmationService.confirm({
        message:
          'You are about to lose all your unsaved changes.<br><br>Are you sure you want to proceed?',
  ...
  onSubmit = () => {
    if (
      ...
      this.task.assignmentTitle = this.taskForm
        .get('assignmentTitle')
        .value.trim();
      ...
      this.task.assignedTo = this.formAssignToArray.controls.map(team => {
        return {
          ...
          teamMemberTaskAssignmentRoleAssociationID:
            team.value.teamMemberTaskAssignmentRoleAssociationID,
      ...
      this.task.workAssignments = this.formWorkAssignmentArray.controls.map(
        workAssignment => {
          return {
            taskResponsibilityTypeDescription: workAssignment.get('type').value,
            taskAssignmentResponsibilityDetail: workAssignment.get('value')
              .value
          };
        }
      );
      this.task.expiredWorkAssignments = this.task.expiredWorkAssignments.map(
        workAssignment => {
          return {
      ...
      this.taskManagementService
        .saveTaskAssignment(this.task, this.updating)
        .subscribe(
          res => {
          ...
          (err: HttpErrorResponse) => {
            ...
            this.messageService.add({
              ...
              detail:
                'There was a problem saving the task assignment. Error: ' +
                detailErrorString
            });
            this.loading = false;
          },
  ...
  private generateInvalidFormErrorMessage() {
    if (!this.taskForm.touched) {
      ...
      this.messageService.add({
        ...
        detail: this.getEmptyFieldsErrorDetail()
      });
    }
  }
  ...
  isCategoryOrTeamsInvalid(): boolean {
    return (
      this.taskForm.get('category').invalid ||
      this.taskForm.get('responsibleTeams').invalid
    );
  }

  validateAddButtons(): boolean {
    return (
      (this.formAssignToArray.touched ||
        this.formWorkAssignmentArray.touched) &&
      this.isCategoryOrTeamsInvalid()
    );
  }
  ...
  fetchTaskCategoryOptions(event?: any) {
    this.taskManagementService.getTaskCategoryOptions().subscribe(res => {
      ...
      for (const element of res) {
        ...
        categoryList.push({
          taskCategoryName: element.taskGroupName,
          taskCategoryId: element.taskGroupID
        });
      }
  ...
  taskCategoryChanged() {
    ...
    if (category) {
      this.taskManagementService
        .getRoleTypeOptions(category.taskCategoryId)
        .subscribe(res => {
          ...
          this.formAssignToArray.controls.forEach((controlGroup: FormGroup) => {
            ...
            if (roleType.value) {
              const filteredRoleTypes = this.userRoleOptions.find(
                roleTypeOption => {
      ...
      this.taskManagementService
        .getWorkAssignmentTypeOptions(category.taskCategoryId)
        .subscribe(res => {
          this.typeOptions = [];
          res.forEach(element => {
            this.typeOptions.push({
              description:
                element.taskResponsibilityType
                  .taskResponsibilityTypeDescription,
              associationId:
                element.taskGroupTaskResponsibilityTypeAssociationID,
              typeCode:
                element.taskResponsibilityType.taskResponsibilityTypeCode
            });
          });
          this.formWorkAssignmentArray.controls.forEach(
              ...
              if (workAssignmentType.value) {
                const filteredOptions = this.typeOptions.find(typeOption => {
                  return (
                    ...
                    workAssignmentType.value.description
                  );
                });
                if (filteredOptions) {
  ...
  searchForOptions(event, workAssignment) {
    ...
    const workAssignmentType =
      workAssignment.type ||
      workAssignmentTypes.find(type => type.name === typeName);
    if (!(workAssignmentType && workAssignmentType.service)) {
      workAssignment.filteredOptions = [];
      workAssignment.get('value').value = workAssignmentType.mapLabelValue(
        event.query
      );
      return;
    }
    if (!workAssignmentType.dropdown) {
      this.fetchWorkAssignmentTypeValues(
        workAssignmentType,
        event.query || ''
      ).subscribe(filteredVals => {
        workAssignment.filteredOptions = filteredVals;
      });
    } else if (event.query !== '') {
      workAssignment.filteredOptions = workAssignment.valueOptions.filter(
        value => value.label.includes(event.query)
      );
    } else {
  ...
  fetchWorkAssignmentTypeValues(
    type: WorkAssignmentType,
    query: string
  ): Observable<any[]> {
    return this.taskManagementService
      .searchForWorkAssignmentTypeValues(type.service, query)
      .pipe(
        map((elements: any[]) => {
  ...
  taskResponsibilityTypeChange(event, workAssignment): any {
    const workAssignmentType = workAssignmentTypes.find(
      type => type.name === event.description
    );
    ...
    if (
      workAssignmentType &&
      workAssignmentType.service &&
      workAssignmentType.dropdown
    ) {
      this.taskManagementService
        .getWorkAssignmentTypeValues(workAssignmentType.service)
        .subscribe(
          (elements: any[]) => {
            if (elements && elements.length > 0) {
          ...
          (err: Error) =>
            this.messageService.add({
              ...
              detail:
                'Error retrieving values for work assignment type: ' +
                workAssignmentType.name +
                '\n' +
                'Error: ' +
                err.message
            })
  ...
  addTeam() {
    ...
    if (this.isCategoryOrTeamsInvalid()) {
      ...
      this.messageService.add({
        ...
        summary: 'Invalid Form',
        detail:
          'Please add a Responsible Team and Category before adding a team.'
      });
      return;
    }
    if (
      this.formAssignToArray.length > 0 &&
  ...
  removeTeams() {
    ...
    this.formAssignToArray.controls = this.formAssignToArray.controls.filter(
      assignTo => !this.selectedAssignedTo.includes(assignTo)
    );
  ...
  findTeamMembers() {
    ...
    this.teamManagementService
      .findTeamMembersByTeamIds(ids)
      .subscribe(teamMemberList => {
    ...
    this.formAssignToArray.controls.forEach((controlGroup: FormGroup) => {
      ...
      if (responsibleTeams.length > 0) {
        responsibleTeams.forEach(responsibleTeam => {
          if (
            assignee.value &&
            assignee.value.id.indexOf('Team-' + responsibleTeam.teamID) !== -1
          ) {
  ...
  addWorkAssignment() {
    ...
    if (this.isCategoryOrTeamsInvalid()) {
      ...
      this.messageService.add({
        ...
        summary: 'Invalid Form',
        detail:
          'Please add a Responsible Team and Category before adding a work assignment.'
      });
      return;
    }
    if (
      this.formWorkAssignmentArray.length > 0 &&
  ...
  removeWorkAssignments() {
    this.taskForm.markAsTouched();
    this.formWorkAssignmentArray.controls = this.formWorkAssignmentArray.controls.filter(
      workAssignment => !this.selectedWorkAssignments.includes(workAssignment)
    );
  ...
  confirmRemoveAssignment() {
    this.confirmationService.confirm({
      message:
        'This function will inactivate the task assignment. Do you want to proceed?',
      accept: () => {
        this.loading = true;
        this.taskManagementService.inactivateTask(this.task.id).subscribe(
          res => {
            if (res === 'Success') {
              this.messageService.add({
                severity: 'success',
  ...
  valueEntered(
    ...
    workAssignment: FormGroup & { type: WorkAssignmentType }
  ) {
    ...
    workAssignment
      .get('value')
      .setValue(workAssignment.type.mapLabelValue(input.value));
    input.value = val;
  }
  ...
  onEditCellClick(cell: CellEditor) {
    ...
    if (
      column.isEnabled() &&
      column.dt.editingCell &&
      column.dt.editingCell !== column.el.nativeElement &&
      !column.isValid()
    ) {
