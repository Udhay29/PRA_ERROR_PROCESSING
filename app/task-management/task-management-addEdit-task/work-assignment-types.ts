...
const defaultLabelValueMap = (input: string): WorkAssignmentMappedValue => ({
  label: input,
  value: input
});

const apptRequirementsMap = [
  {
...
function formatOrganizationAddress(source: any): string {
  let totalAddress = source.OrganizationName
...
export const workAssignmentTypes: WorkAssignmentType[] = [
  ...
  {
    name: 'Appointment Requirements',
    code: 'ApptRqmts',
    service: {
      destination: URLS.destinationSite,
      method: HTTPMethod.NONE,
      response_map: () => apptRequirementsMap
    },
    mapLabelValue: (element: any) => element,
    dropdown: true
  },
  {
    name: 'Bill To',
  ...
  },
  {
    ...
    service: {
      ...
      query_string: (query: string) =>
        `?areaName=${query}&areaType=capacity&projection=area`,
  ...
  {
    name: 'Destination State or Province',
    code: 'DestState',
    service: {
      destination: URLS.statesUrl,
      method: HTTPMethod.GET,
      response_map: (res: any) => res._embedded.states
    },
    mapLabelValue: (element: any) => ({
      label: `${element.stateName} - ${element.stateCode}`,
  ...
  },
  {
    ...
    mapLabelValue: (element: any) => ({
      ...
      value: element._source.OperationalGroupCode
    })
  },
  {
  ...
  },
  {
    ...
    code: 'OpratnlGrp',
    service: {
      destination: URLS.operationalGroupService,
      method: HTTPMethod.GET,
      query_string: (input: string) =>
        `?fleetcode=${input}&rowCount=25&start=0&end=20`,
