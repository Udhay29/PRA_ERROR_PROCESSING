...
export class EmployeesComponent implements OnInit {
    ...
    this.route.paramMap.pipe(
      ...
      if (this.isEmployee(result)) {
        ...
        this.delegations = result.delegations;
          this.pictureService.getProfilePicture(result.userName).subscribe( pic => {
          this.selectedEmp.profilePic = pic ? pic : 'assets/images/nouser.png';
        });
  ...
  mapEmployeesForTable( employees: Employee[] ): TableEmployee[] {
    return employees.map( (emp: Employee) => {
      return {
        ...
        title: emp.title,
        teams: emp.teams.filter( (teamObj: Team) => moment(teamObj.expirationTimestamp).isAfter(moment.now()))
                        .map( (team: Team) => team.name).join(', '),
  ...
  loadEmployee( id: number ) {
    this.employeesService.getEmployee( id ).pipe(
      ...
      this.pictureService.getProfilePicture(emp.userName).subscribe( pic => {
        this.selectedEmp.profilePic = pic ? pic : 'assets/images/nouser.png';
      });
